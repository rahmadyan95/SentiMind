...
class YoutubeScrapperView(ctk.CTkFrame):
    ...
    def left_side(self):
        ...
        self.LinkPlaceholder = ctk.CTkLabel(link_box, font=('FiraCode Nerd Font',18),text="Youtube Link :",bg_color='grey30')
    ...
    def show_table(self):
        ...
        result = self.InstancesControler.get_data(self.YoutubeLinkInput.get(), num_of_commnet)
        
        cleaned_comments = result['cleaned_comments']
        values = [[comment] for comment in cleaned_comments]
        row_count = len(values)

        self.link_place.configure(text=f"{self.YoutubeLinkInput.get()}")
